<?php
	...
	if(!empty($token["time"]) && time() - $token["time"] > 604800) {
		...
		setcookie("anu-notes-remember-me", null, -1, "/");
	}
	if(isset($_COOKIE['anu-notes-remember-me']) && $_COOKIE['anu-notes-remember-me'] == $token["key"] && !empty($token["time"])) {
		$token_valid = true;
	}
	...
	if($action == "login") {
		if(!empty($_POST['username']) && !empty($_POST['password'])) {			
			...
			if(strtolower($username) == strtolower($valid_username) && password_verify($password, $valid_password)) {
				...
				if($remember == "true") {
					$token["time"] = time();
					setcookie("anu-notes-remember-me", $token["key"], time() + 604800, "/");
				}
				else {
					$token["time"] = "";
					setcookie("anu-notes-remember-me", null, -1, "/");
